...
export class DicomViewerComponent implements OnInit {
  ...
  viewPort: CornerstoneDirective; // the main cornerstone view port
  ...
  ngOnInit() {
    ...
    console.log('this.enableViewerTools', this.enableViewerTools);
  ...
  public loadMoreImages() {
    this.element = this.viewPort.element;
    //
    // loop thru all imageIds
    // load and cache them for exhibition(up the the maximum limit defined)
    //
    const maxImages =
      this.maxImagesToLoad <= 0
        ? this.imageIdList.length - this.loadedImages.length
  ...
  private imageLoaded(imageData) {
    ...
    // build list of series in all loaded images
    const series = {
    ...
    if (seriesIndex < 0) {
      ...
      const seriesItem = this.seriesList[seriesIndex];
